...
get_temp =u.api_uri+"temp/list"
get_csv = u.api_uri+"temp/csv"
get_pdf = u.api_uri+"temp/pdf"
get_excel = u.api_uri+"temp/excel"
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.logo_image_export_data = ctk.CTkImage(Image.open(os.path.join(image_path, "export_hitam.png")), size=(40, 40))
        self.logo_image_clear = ctk.CTkImage(Image.open(os.path.join(image_path, "dust_hitam.png")), size= (40,40))
        ...
        self.name_of_page = ctk.CTkLabel(master=self.main_frame,text="Temperature Log", text_color="#abd1c6",font=ctk.CTkFont(size=25, weight="bold"))
        ...
        self.btn_export_data = ctk.CTkButton(master=self.main_frame,width=100,height=70,text="Export",text_color="#000000", fg_color="#abd1c6",corner_radius=20,font=ctk.CTkFont(size=12,weight="bold"), image=self.logo_image_export_data,compound="top", anchor="c", command= self.download_file)
        ...
        self.btn_back = ctk.CTkButton(master=self.main_frame,width=80,height=50,text="BACK",text_color="#000000", fg_color="#abd1c6",corner_radius=10,font=ctk.CTkFont(size=20,weight="bold"), command=self.btnback)
        ...
        for i in range(0, self.jumlah_data, self.rows_per_page):
            ...
            for j in range(self.rows_per_page):
                if i + j < self.jumlah_data:
                    page_data.append([self.date_log[i + j],self.time_log[i + j],self.panel_temp[i + j], self.emlock_temp[i + j]])
    ...
    def update_table(self):
        self.panel_temp.clear()
        self.emlock_temp.clear()
        self.id_log.clear()
        del self.time_log
        del self.date_log
        self.jumlah_data = 0
        
        response = requests.get(get_temp,headers={"Authorization":u.Authorization})
        data = response.json()  
        data = data[:10]
        ...
        self.rows_per_page = u.row_per_pages
        self.panel_temp = [item.get('panel_temp', 'Unknown') for item in data]
        self.emlock_temp = [item.get('emlock_temp', 'Unknown') for item in data]
        ...
        self.head = ["Date", "Time", "Panel Temperature", "EM Lock Temperature"]
        ...
        for i, page_data in enumerate(self.pages, start=1):
            ...
            table = CTkTable(master=tab_frame, row=len(page_data) + 1, column=len(self.head),
                             ...
                             justify="center", hover_color="#f9bc60",font=ctk.CTkFont(size=17))
    ...
    def create_csv(self):
        data =[]
        data.extend([list(a) for a in zip(self.date_log, self.time_log, self.name_user, self.job_user, self.platform_user, self.action_user)])
        file_name = "Data Log :"+datetime.today().strftime('%Y-%m-%d %H:%M')+".csv"
        print(data)
        print(file_name)
        with open("../../Documents/Data_Log/Door_Access/"+file_name, 'w') as csvfile:
            header =["Date", "Time", "Panel Temperature", "EM Lock Temperature"]
    ...
    def btnback (self):
        self.destroy()
        import History_page_master_admin as dma
        dma.main() 
