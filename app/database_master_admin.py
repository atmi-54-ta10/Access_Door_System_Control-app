...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.logo_image_delete = ctk.CTkImage(Image.open(os.path.join(image_path, "delete-user hitam.png")), size=(100, 100))

        #inisiasi gambar edit data
        self.logo_image_edit = ctk.CTkImage(Image.open(os.path.join(image_path, "database hitam.png")), size=(100, 100))

        #inisiasi gambar Data logging
        self.logo_image_datalogging = ctk.CTkImage(Image.open(os.path.join(image_path, "Data_logging hitam.png")), size=(100, 100))
        ...
        self.clock_frame.place(relx=0.75, rely=0.005)
        ...
        self.name_of_page = ctk.CTkLabel(master=self.main_frame,text="Database", text_color="#abd1c6",font=ctk.CTkFont(size=25, weight="bold"))
        self.name_of_page.place(relx=0.01, rely=0.01)

        #button add data
        self.btn_add_data = ctk.CTkButton(master=self.main_frame,width=200,height=200,text="ADD",text_color="#004643", fg_color="#abd1c6",corner_radius=20,font=ctk.CTkFont(size=20,weight="bold"), image=self.logo_image_adddata,compound="top", command=self.add_data)
        self.btn_add_data.place(relx=0.75, rely=0.16)

        #button delete data
        self.btn_delete_data = ctk.CTkButton(master=self.main_frame,width=200,height=200,text="DELETE",text_color="#004643", fg_color="#abd1c6",corner_radius=20,font=ctk.CTkFont(size=20,weight="bold"), image=self.logo_image_delete,compound="top", command=self.del_data)
        self.btn_delete_data.place(relx=0.64, rely=0.55)

        #  #button data logging
        # self.btn_datalogging = ctk.CTkButton(master=self.main_frame,width=200,height=200,text="DATA LOG",text_color="#000000", fg_color="#abd1c6",corner_radius=20,font=ctk.CTkFont(size=20,weight="bold"), image=self.logo_image_datalogging,compound="top")
        # self.btn_datalogging.place(relx=0.75, rely=0.55)

        #button edit data
        self.btn_edit_data = ctk.CTkButton(master=self.main_frame,width=200,height=200,text="EDIT DATA",text_color="#004643", fg_color="#abd1c6",corner_radius=20,font=ctk.CTkFont(size=20,weight="bold"), image=self.logo_image_edit,compound="top", command= self.edit_data)
    ...
    def add_data(self):
        self.destroy()
        import dt_add_master_admin as adma
        adma.main()

    def del_data(self):
        self.destroy()
        import dt_del_master_admin as dma
        dma.main()

    def edit_data(self):
        self.destroy()
        import dt_edit_master_admin as ema
        ema.main()
