...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.logo_image_delete_data = ctk.CTkImage(Image.open(os.path.join(image_path, "edit_putih.png")), size=(75, 75))
        ...
        self.name_of_page = ctk.CTkLabel(master=self.main_frame,text="Edit data", text_color="#abd1c6",font=ctk.CTkFont(size=25, weight="bold"))
        self.name_of_page.place(relx=0.01, rely=0.01)

        self.frame_kanan = ctk.CTkFrame(master=self.main_frame, height=465, width=400,fg_color="#abd1c6")
        ...
        self.name = ctk.CTkEntry(master=self.frame_kanan, textvariable=self.name_slct,height=30,width=350,fg_color="#abd1c6", corner_radius=25, text_color="#004643",border_color="#004643",font=ctk.CTkFont(size=15),state="normal")
        ...
        self.combobox_job = ctk.CTkComboBox(master=self.frame_kanan,values=job,height=30, width=350, fg_color="#abd1c6",text_color="#004643",dropdown_fg_color="#004643",dropdown_text_color="#abd1c6",corner_radius=25,font=ctk.CTkFont(size=15),dropdown_font=ctk.CTkFont(size=17))
        self.combobox_job.set(value="")  
        self.combobox_job.place(relx= 0.04, rely= 0.24)
       
        self.ni = ctk.CTkEntry(master=self.frame_kanan, textvariable=self.id_number_slct,height=30,width=350,fg_color="#abd1c6", corner_radius=25, text_color="#004643",border_color="#004643",font=ctk.CTkFont(size=15),state="normal")
        self.ni.place(relx=0.04, rely= 0.38)
        self.label_ni = ctk.CTkLabel(master=self.frame_kanan, text="ID Number", font=ctk.CTkFont(size=17, weight="bold"),text_color="#004643")
        self.label_ni.place(relx=0.06, rely=0.32)
        #tombol 
        self.btn_edit_data = ctk.CTkButton(master=self.frame_kanan,width=150,height=150,text="Edit",text_color="#abd1c6", fg_color="#004643",corner_radius=20,font=ctk.CTkFont(size=20,weight="bold"), image=self.logo_image_delete_data,compound="top", anchor="c", command= self.edit_data)
        self.btn_edit_data.place(relx=0.1, rely=0.5) 

        self.btn_clear_data = ctk.CTkButton(master=self.frame_kanan,width=150,height=150,text="Clear",text_color="#abd1c6", fg_color="#004643",corner_radius=20,font=ctk.CTkFont(size=20,weight="bold"), image=self.logo_image_clear,compound="top",command=self.clear)
        self.btn_clear_data.place(relx=0.5, rely=0.5)

        self.btn_scan_card = ctk.CTkButton(master=self.frame_kanan,width=100,height=50,text="Scan Card",text_color="#abd1c6", fg_color="#004643",corner_radius=20,font=ctk.CTkFont(size=15,weight="bold"), command= self.scan_rfid)
    ...
    async def on_scan_rfid(self):
        ...
        if nama != "":
            if self.nama_asli in self.name_user:
                no_id = self.name_user.index(self.nama_asli)
                id_USER = self.id_user[no_id]
                data = await self.scan_rfid_async(scan_card_url+id_USER)
